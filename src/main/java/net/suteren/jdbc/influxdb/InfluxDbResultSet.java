...
public class InfluxDbResultSet implements ResultSet {
	...
	public InfluxDbResultSet(QueryResult results) {
		this.results = results.getResults();
		this.error = results.getError();
		this.hasError = results.hasError();
	}

	@Override public boolean next() throws SQLException {
		return position.intValue() < results.size();
	}
	...
	@Override public String getString(int columnIndex) throws SQLException {
		QueryResult.Result result = results.get(position.intValue());
		return result.getSeries().get(0).getColumns().get(columnIndex);
	}
